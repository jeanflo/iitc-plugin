...
function imattcMain() {
    function loadDependencies(callback) {
        var jqLoaded = typeof window.$ !== 'undefined' && window.$.fn && window.$.fn.jquery;
        if (!jqLoaded) {
            var insertJq = document.createElement('script');
            insertJq.type = 'text/javascript';
            insertJq.src = 'https://code.jquery.com/jquery-1.12.4.min.js';
        ...
        var checkJq = setInterval(function() {
            if (typeof window.$ !== 'undefined' && window.$.fn && window.$.fn.jquery) {
                ...
                if (!window.$.ui) {
                    var insertJqUi = document.createElement('script');
                    insertJqUi.type = 'text/javascript';
                    insertJqUi.src = 'https://code.jquery.com/ui/1.10.4/jquery-ui.min.js';
                    document.head.appendChild(insertJqUi);
                }
                
                var checkJqUi = setInterval(function() {
                    if (window.$.ui) {
                        clearInterval(checkJqUi);
                        callback();
                    }
                }, 100);
            }
...
script.appendChild(document.createTextNode('(' + imattcMain + ')();'));
